        ...
        <ul class="navbar-nav mr-auto">
            ...
            <li class="nav-item dropdown">
                ...
                <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                    <a class="dropdown-item" href="<c:url value='/add_composition'/>">Add new composition</a>
        ...
        <%
            ...
            try {
                ...
                if(request.getParameter("id")!=null)
                    comps = CompositionDao.findByAlbumId(hibernateSession,Long.parseLong(request.getParameter("id")));
                else
                    comps = CompositionDao.findAll(hibernateSession);
                if(comps != null) {
                    out.println("<tr><th scope=\"col\">#</th><th scope=\"col\">Name</th><th scope=\"col\">Album</th><th scope=\"col\">Action</th></tr>");
                    for(int i = 0; i < comps.size(); i++) {
        %>
        <tr>
            <td><%=(i+1)%></td>
            <td><%=comps.get(i).getName()%></td>
            <td><%=comps.get(i).getAlbum().getName()%></td>
            <td>
                <form action="admin_composition_action" method="POST">
                    <input type="hidden" name="id" value="<%=comps.get(i).getId()%>">
                    <input type="hidden" name="name" value="<%=comps.get(i).getName()%>">
